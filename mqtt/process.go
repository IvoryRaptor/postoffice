...
func (s *service) processor() {
	defer func() {
		...
		//glog.Debugf("(%s) Stopping processor", s.cid())
	}()
	...
	for {
		...
		//glog.Debugf("(%s) Received: %s", s.cid(), msg)
		...
		//if s.inStat.msgs%1000 == 0 {
		//	glog.Debugf("(%s) Going to process message %d", s.cid(), s.inStat.msgs)
		//}
	}
}
...
func (s *service) processAcked(ackq *sessions.Ackqueue) {
	for _, ackmsg := range ackq.Acked() {
		...
		//glog.Debugf("(%s) Processing acked message: %v", s.cid(), ack)
		...
		case message.PUBACK, message.PUBCOMP, message.SUBACK, message.UNSUBACK, message.PINGRESP:
			...
			// If ack is PUBACK, that means the QoS 1 message sent by s mqtt got
			// ack'ed. There's nothing to do other than calling onComplete() below.

			// If ack is PUBCOMP, that means the QoS 2 message sent by s mqtt got
			// ack'ed. There's nothing to do other than calling onComplete() below.

			// If ack is SUBACK, that means the SUBSCRIBE message sent by s mqtt
			// got ack'ed. There's nothing to do other than calling onComplete() below.

			// If ack is UNSUBACK, that means the SUBSCRIBE message sent by s mqtt
			// got ack'ed. There's nothing to do other than calling onComplete() below.

			// If ack is PINGRESP, that means the PINGREQ message sent by s mqtt
			// got ack'ed. There's nothing to do other than calling onComplete() below.
...
func (s *service) processPublish(msg *message.PublishMessage) error {
	println(s.sess.ID(), "publish -> ", string(msg.Topic()))
	switch msg.QoS() {
...
func (s *service) processSubscribe(msg *message.SubscribeMessage) error {
	...
	for _, rm := range s.rmsgs {
		if err := s.publish(rm, nil); err != nil {
			glog.Errorf("mqtt/processSubscribe: Error publishing retained message: %v", err)
			return err
		}
	}
...
func (s *service) onPublish(msg *message.PublishMessage) error {
	...
	//glog.Debugf("(%s) Publishing to topic %q and %d subscribers", s.cid(), string(msg.Topic()), len(s.subs))
