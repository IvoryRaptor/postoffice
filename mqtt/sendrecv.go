...
func (s *service) receiver() {
	...
	case net.Conn:
		//glog.Debugf("server/handleConnection: Setting read deadline to %d", time.Second*time.Duration(s.keepAlive))
	...
	//case *websocket.Conn:
	//	glog.Errorf("(%s) Websocket: %v", s.cid(), ErrInvalidConnectionType)
...
func (s *service) sender() {
	...
	//case *websocket.Conn:
	//	glog.Errorf("(%s) Websocket not supported", s.cid())
...
func (s *service) writeMessage(msg message.Message) (int, error) {
	...
	// another connetion, and the message needs to be published to s client, then
	...
	// to s client, then they will all block. However, s will do for now.
	//
