...
type service struct {
	// The ID of this mqtt, it's not related to the Client ID, just a number that's
	// incremented for every new mqtt.
	id uint64
	...
	// Network connection for this mqtt
	conn io.Closer
	...
	// Whether this is mqtt is closed or not.
	closed int64

	// Quit signal for determining when this mqtt should end. If channel is closed,
	// then exit.
	done chan struct{}
	...
	if !s.client {
		...
		s.onpub = func(msg *message.PublishMessage) error {
			if err := s.publish(msg, nil); err != nil {
				glog.Errorf("mqtt/onPublish: Error publishing message: %v", err)
				return err
			}
		...
		// If s is a recovered session, then add any topics it subscribed before
...
func (s *service) stop() {
	...
	if s.done != nil {
		glog.Debugf("(%s) closing s.done", s.cid())
		close(s.done)
	}

	// Close the network connection
	if s.conn != nil {
		glog.Debugf("(%s) closing s.conn", s.cid())
		s.conn.Close()
	}
	...
	// Unsubscribe from all the topics for s client, only for the server side though
	...
	if !s.client && s.sess.Cmsg.WillFlag() {
		glog.Infof("(%s) mqtt/stop: connection unexpectedly closed. Sending Will.", s.cid())
...
func (s *service) publish(msg *message.PublishMessage, onComplete OnCompleteFunc) error {
	//glog.Debugf("mqtt/publish: Publishing %s", msg)
...
func (s *service) unsubscribe(msg *message.UnsubscribeMessage, onComplete OnCompleteFunc) error {
	...
	var onc OnCompleteFunc = func(msg, ack message.Message, err error) error {
		...
		for _, tb := range unsub.Topics() {
			// Remove all subscribers, which basically it's just s client, since
